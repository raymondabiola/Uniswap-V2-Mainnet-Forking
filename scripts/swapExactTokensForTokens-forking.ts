...
//   const amountUSDCMin = ethers.parseUnits("9000", 6);
  const amountDAIMin = ethers.parseUnits("900", 18);
  ...
  console.log("DAI Balance before swapping:", ethers.formatUnits(daiBalBefore, 18));

await USDC.connect(impersonatedSigner).approve(UNIRouter, amountUSDC);
//   await DAI.approve(UNIRouter, amountDAI);
  ...
  console.log("DAI Balance after swapping:", ethers.formatUnits(daiBalAfter, 18));
